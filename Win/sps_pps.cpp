...
//#define P264_TRACE  printf
//#define P264_TRACE_ADDRESS() P264_TRACE("@%d ", (s->p - s->p_start+1)*8 - s->i_left); 


void _TRACE2(char* szFormat, ...)
{
#ifdef _DEBUG2
	char buff[1024] = {0};
	va_list args;
...
//            s->avctx->aspect_ratio= sar_width*s->width / (float)(s->height*sar_height);
    ...
    if(sps->vui.b_timing_info_present){
        ...
        if(!sps->vui.i_num_units_in_tick || !sps->vui.i_time_scale){
            //_TRACE2("time_scale/num_units_in_tick invalid or unsupported (%d/%d)\n", sps->vui.i_time_scale, sps->vui.i_num_units_in_tick);
            return -1;
        }
#ifdef _DEBUG1
    ...
    if(sps->vui.b_bitstream_restriction){
        ...
        if( (unsigned)sps->vui.i_num_reorder_frames > 16U /*max_dec_frame_buffering || max_dec_frame_buffering > 16*/){
            //_TRACE2("illegal num_reorder_frames %d\n", sps->vui.i_num_reorder_frames);
            return -1;
        }
    }
    if (bs_eof(s)) {
        //_TRACE2("Overread VUI by bits\n");
        return -1;
    }
    ...
    if(sps->i_profile_idc >= 100){ //high profile
        ...
        //decode_scaling_matrices(s, sps, NULL, 1, sps->scaling_matrix4, sps->scaling_matrix8);
            ...
            for( int i = 0; i < ((3 != sps->i_chroma_format_idc) ? 8 : 12); i++ )
            {
				...
				for (int i=0; i< ((sps->i_chroma_format_idc!=3)?8:12); i++)
				{
					int seq_scaling_list_present_flag = bs_read( s, 1 );
...
int h264_pps_read( unsigned char *nal, int nal_len, h264_pps_t *pps )
{
    ...
    //_TRACE2("PPS: pic_parameter_set_id = %d\n", id);
    if( bs_eof( s ) || id >= 256 )
    {
    ...
    //_TRACE2("PPS: seq_parameter_set_id = %d\n", pps->i_sps_id);
    if( pps->i_sps_id >= 32 )
    {
        goto error;
    }
...
//        //ËÉÏÂ(WV-SP306 FW1.44)-720P.h264
