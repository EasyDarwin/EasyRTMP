...
void  h265_read_sps_rbsp(h265_stream_t* h, bs_t* b, float *fps)
{
    ...
    if (sps->vui_parameters_present_flag)
    {
		...
		if (NULL!=fps)	*fps = (float)(sps->vui.vui_time_scale) / (float)(sps->vui.vui_num_units_in_tick);
    }
...
void h265_read_ptl2(bs_t* b, int profilePresentFlag, int max_sub_layers_minus1)
{
    int i = 0;
    if (profilePresentFlag)
    {
        uint8_t	general_profile_space = bs_read_u(b, 2);
        uint8_t general_tier_flag     =  bs_read_u1(b);
        uint8_t general_profile_idc   = bs_read_u(b, 5);
		uint8_t general_profile_compatibility_flag[32] = {0,};
        for (i = 0; i < 32; i++)
        {
            general_profile_compatibility_flag[i] = bs_read_u1(b);
        }
        uint8_t general_progressive_source_flag    = bs_read_u1(b);
        uint8_t general_interlaced_source_flag     = bs_read_u1(b);
        uint8_t general_non_packed_constraint_flag = bs_read_u1(b);
        uint8_t general_frame_only_constraint_flag = bs_read_u1(b);
    ...
    uint8_t general_level_idc = bs_read_u8(b);
	uint8_t *sub_layer_profile_present_flag = new uint8_t[max_sub_layers_minus1];
	uint8_t *sub_layer_level_present_flag = new  uint8_t[max_sub_layers_minus1];
    //ptl->sub_layer_profile_present_flag.resize(max_sub_layers_minus1);
    //ptl->sub_layer_level_present_flag.resize(max_sub_layers_minus1);
    for (i = 0; i < max_sub_layers_minus1; i++)
    {
        bs_read_u1(b);
        sub_layer_level_present_flag[i]   = bs_read_u1(b);
    }
    if (max_sub_layers_minus1 > 0)
    {
        for (i = max_sub_layers_minus1; i < 8; i++)
	...
	*/
    for (i = 0; i < max_sub_layers_minus1; i++)
    {
        if (sub_layer_profile_present_flag[i])
        {
            bs_read_u(b, 2);
            bs_read_u1(b);
            sub_layer_profile_idc[i]   = bs_read_u(b, 5);
			uint8_t *sub_layer_profile_compatibility_flag = new uint8_t[32];
            for (int j = 0; j < 32; j++)
            {
                sub_layer_profile_compatibility_flag[j] = bs_read_u1(b);
            }
            bs_read_u1(b);
            bs_read_u1(b);
            bs_read_u1(b);
            bs_read_u1(b);
            if (sub_layer_profile_idc[i]==4 || sub_layer_profile_compatibility_flag[4] ||
                sub_layer_profile_idc[i]==5 || sub_layer_profile_compatibility_flag[5] ||
                sub_layer_profile_idc[i]==6 || sub_layer_profile_compatibility_flag[6] ||
                sub_layer_profile_idc[i]==7 || sub_layer_profile_compatibility_flag[7])
            {
                bs_read_u1(b);
                bs_read_u1(b);
                bs_read_u1(b);
...
void h265_read_sps_rbsp2(unsigned char *nal, int nal_len, int *width, int *height, float *fps)
{
    bs_t bs;
    bs_t *s = &bs;
	...
	if (nal_len < 0)			return;
/*
    int forbidden_zero_bit = bs_read_u(s,1);
    int nal_unit_type = bs_read_u(s,6);
    int nuh_layer_id = bs_read_u(s,6);
    int nuh_temporal_id_plus1 = bs_read_u(s,3);
*/
    int rbsp_size = nal_len;
    uint8_t* rbsp_buf = (uint8_t*)malloc(rbsp_size);
	int rc = nal_to_rbsp(2, nal, &nal_len, rbsp_buf, &rbsp_size);
	if (rc < 0) { free(rbsp_buf); return ; } // handle conversion error
	 /*
	FILE *f = fopen("s.txt", "wb");
	if (NULL != f)
	{
		fwrite(rbsp_buf, 1, rbsp_size, f);
		fclose(f);
	}
	*/
	if ( (rbsp_size > 4) && ( (unsigned char)rbsp_buf[1]==0x01 && (unsigned char)rbsp_buf[2] == 0x42) )
	{
		for (int i=0; i<rbsp_size-4; i++)
		{
			rbsp_buf[i] = rbsp_buf[i+4];
		}
		rbsp_size -= 4;
	}
    ...
    sps_video_parameter_set_id = bs_read_u(s, 4);
    sps_max_sub_layers_minus1 = bs_read_u(s, 3);
    sps_temporal_id_nesting_flag = bs_read_u1(s);

    // profile tier level...
    //memset(&profile_tier_level, 0x00, sizeof(profile_tier_level_t));

    profile_tier_level.general_profile_space=0;
    profile_tier_level.general_tier_flag=0;
    profile_tier_level.general_profile_idc=0;
	memset(&profile_tier_level.general_profile_compatibility_flag[0], 0x00, sizeof(profile_tier_level.general_profile_compatibility_flag));
    profile_tier_level.general_progressive_source_flag=0;
    profile_tier_level.general_interlaced_source_flag=0;
    profile_tier_level.general_non_packed_constraint_flag=0;
    profile_tier_level.general_frame_only_constraint_flag=0;
    profile_tier_level.general_max_12bit_constraint_flag=0;
    profile_tier_level.general_max_10bit_constraint_flag=0;
    profile_tier_level.general_max_8bit_constraint_flag=0;
    profile_tier_level.general_max_422chroma_constraint_flag=0;
    profile_tier_level.general_max_420chroma_constraint_flag=0;
    profile_tier_level.general_max_monochrome_constraint_flag=0;
    profile_tier_level.general_intra_constraint_flag=0;
    profile_tier_level.general_one_picture_only_constraint_flag=0;
    profile_tier_level.general_lower_bit_rate_constraint_flag=0;
    profile_tier_level.general_reserved_zero_34bits=0; // todo
    profile_tier_level.general_reserved_zero_43bits=0; // todo
    profile_tier_level.general_inbld_flag=0;
    profile_tier_level.general_reserved_zero_bit=0;
    profile_tier_level.general_level_idc=0;
    profile_tier_level.sub_layer_profile_present_flag.clear();
    profile_tier_level.sub_layer_level_present_flag.clear();
	memset(&profile_tier_level.reserved_zero_2bits[0], 0x00, sizeof(profile_tier_level.reserved_zero_2bits));
    profile_tier_level.sub_layer_profile_space.clear();
    profile_tier_level.sub_layer_tier_flag.clear();
    profile_tier_level.sub_layer_profile_idc.clear();
    profile_tier_level.sub_layer_profile_compatibility_flag.clear();
    profile_tier_level.sub_layer_progressive_source_flag.clear();
    profile_tier_level.sub_layer_interlaced_source_flag.clear();
    profile_tier_level.sub_layer_non_packed_constraint_flag.clear();
    profile_tier_level.sub_layer_frame_only_constraint_flag.clear();
    profile_tier_level.sub_layer_max_12bit_constraint_flag.clear();
    profile_tier_level.sub_layer_max_10bit_constraint_flag.clear();
    profile_tier_level.sub_layer_max_8bit_constraint_flag.clear();
    profile_tier_level.sub_layer_max_422chroma_constraint_flag.clear();
    profile_tier_level.sub_layer_max_420chroma_constraint_flag.clear();
    profile_tier_level.sub_layer_max_monochrome_constraint_flag.clear();
    profile_tier_level.sub_layer_intra_constraint_flag.clear();
    profile_tier_level.sub_layer_one_picture_only_constraint_flag.clear();
    profile_tier_level.sub_layer_lower_bit_rate_constraint_flag.clear();
    profile_tier_level.sub_layer_reserved_zero_34bits.clear();
    profile_tier_level.sub_layer_reserved_zero_43bits.clear();
    profile_tier_level.sub_layer_inbld_flag.clear();
    profile_tier_level.sub_layer_reserved_zero_bit.clear();
    profile_tier_level.sub_layer_level_idc.clear();
	h265_read_ptl(&profile_tier_level, s, 1, sps_max_sub_layers_minus1);

	sps_seq_parameter_set_id = bs_read_ue(s);
	int chroma_format_idc = bs_read_ue(s);
    //h->info->chroma_format_idc = sps.chroma_format_idc;
    if (chroma_format_idc == 3)
    {
        //sps.separate_colour_plane_flag = bs_read_u1(s);
		bs_read_u1(s);
    }
    int pic_width_in_luma_samples  = bs_read_ue(s);
    int pic_height_in_luma_samples = bs_read_ue(s);
    //h265_read_ptl2(s, 1, sps_max_sub_layers_minus1);

	if (NULL != width)		*width = pic_width_in_luma_samples;
	if (NULL != height)		*height = pic_height_in_luma_samples;
